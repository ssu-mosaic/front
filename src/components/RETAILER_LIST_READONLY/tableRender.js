...
function RetailerListTable() {
  ...
  useEffect(() => {
    ...
    axios.post(`${baseURL}/retailer/list`, userData).then((response) => {
  ...
  const onSubmitOrderClick = () => {
    ...
    //     const data = await response.data;
    //     console.log(data);
    axios.post(`${baseURL}/order/add`, submitData).then((response) => {
    ...
    <div>
      {loading ? (
        ...
        <Fragment>
          ...
          {request ? (
            <Fragment>
              <div className={`${resultStyles.request__name}`}>주문상세</div>
